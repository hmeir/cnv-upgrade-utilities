...
def fetch_source_version(
    ...
    minor_offset: int | None = None,
    source_minor_version: str | None = None,
) -> BuildResult:
    """Fetch source version build info using GetReleasedBuilds.

    For cross-major Y-stream, pass source_minor_version explicitly (e.g., "4.22")
    to bypass minor_offset arithmetic that would produce invalid versions.
    """
    if source_minor_version is not None:
        return find_released_source(explorer=explorer, minor_version=f"v{source_minor_version}")
...
def get_upgrade_paths_info(
    ...
    cross_major_sources = CROSS_MAJOR_Y_STREAM_SOURCES.get(target_version_str, [])
    upgrade_lanes: dict[str, dict] = {}

    for upgrade_type in upgrade_types:
        if upgrade_type == UpgradeType.Y_STREAM and cross_major_sources:
            for source_ver in cross_major_sources:
                label = f"{upgrade_type.display_name} ({source_ver})"
                upgrade_lanes[label] = ReleaseChecklistUpgradeEntry.from_build_result(
                    build_result=fetch_source_version(
                        explorer=explorer, target_version=target_version, source_minor_version=source_ver
                    ),
                    post_upgrade_suite=get_post_upgrade_suite(
                        upgrade_type=upgrade_type,
                        z=target_version.micro,
                        minor=target_version.minor,
                        major=target_version.major,
                    ),
                ).to_dict()
        else:
            upgrade_lanes[upgrade_type.display_name] = ReleaseChecklistUpgradeEntry.from_build_result(
                build_result=fetch_source_version(
...
@click.option(
    ...
    help="Target version in format X.Y.z (e.g., 4.20.2, 5.0.0)",
)
@click.option(
    "--skip-target-check",
    is_flag=True,
