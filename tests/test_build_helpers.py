...
class TestMakeBuildResult:
    def test_basic(self):
        result = make_build_result(
            version="4.22.2",
            bundle_version="4.22.2.rhel9-31",
            iib="registry/iib:123",
            channel="stable",
        )
        assert isinstance(result, BuildResult)
        assert result.version == "4.22.2"
        assert result.bundle_version == "4.22.2.rhel9-31"
...
class TestExtractFilteredBuildInfo:
    def test_extract(self):
        build = make_successful_build(
            cnv_build="v4.22.2.rhel9-31",
            iib="registry/iib:123",
            channel="stable",
            released_to_prod=True,
            in_stage=False,
        )
        result = extract_filtered_build_info(build=build, version="4.22.2")
        assert result.version == "4.22.2"
        assert result.bundle_version == "4.22.2.rhel9-31"
        assert result.iib == "registry/iib:123"
        assert result.channel == "stable"
    ...
    def test_strips_v_prefix(self):
        build = make_successful_build(cnv_build="v4.22.2.rhel9-31")
        result = extract_filtered_build_info(build=build, version="4.22.2")
        assert result.bundle_version == "4.22.2.rhel9-31"
...
class TestExtractBuildInfoResult:
    def test_extract(self):
        ...
        bi = make_build_info(cnv_version="v4.22.2.rhel9-31", channels=channels)
    ...
    def test_strips_v_prefix_and_rhel_suffix(self):
        ...
        channels = [make_channel_info(channel="stable")]
        bi = make_build_info(cnv_version="v4.22.0.rhel9-234", channels=channels)
        result = extract_from_build_info(build_info=bi, channel="stable")
        assert result.version == "4.22.0"
        assert result.bundle_version == "4.22.0.rhel9-234"
...
class TestFindStableStageBuild:
    def test_found_not_released(self, mock_explorer):
        ...
        result = find_stable_stage_build(explorer=mock_explorer, version="4.22.2")
...
class TestFindReleasedSource:
    def test_found_stable_prod(self, mock_explorer):
        ...
        build = make_released_build(csv_version="v4.22.2", version="v4.22.2.rhel9-31", channels=channels)
        mock_explorer.get_released_builds.return_value = [build]
        result = find_released_source(explorer=mock_explorer, minor_version="v4.22")
        assert result.version == "4.22.2"

    def test_with_required_csv_match(self, mock_explorer):
        channels = [make_channel_info(channel="stable", released_to_prod=True)]
        build = make_released_build(csv_version="v4.22.0", channels=channels)
        mock_explorer.get_released_builds.return_value = [build]
        result = find_released_source(explorer=mock_explorer, minor_version="v4.22", required_csv="v4.22.0")
        assert result.version == "4.22.0"

    def test_with_required_csv_no_match_raises(self, mock_explorer):
        channels = [make_channel_info(channel="stable", released_to_prod=True)]
        build = make_released_build(csv_version="v4.22.2", channels=channels)
        mock_explorer.get_released_builds.return_value = [build]
        with pytest.raises(ValueError, match="No stable build released to prod found for source version"):
            find_released_source(explorer=mock_explorer, minor_version="v4.22", required_csv="v4.22.0")

    def test_with_exclude_csv(self, mock_explorer):
        channels = [make_channel_info(channel="stable", released_to_prod=True)]
        build_excluded = make_released_build(csv_version="v4.22.5", channels=channels)
        build_ok = make_released_build(csv_version="v4.22.2", version="v4.22.2.rhel9-31", channels=channels)
        mock_explorer.get_released_builds.return_value = [build_excluded, build_ok]
        result = find_released_source(explorer=mock_explorer, minor_version="v4.22", exclude_csv="4.22.5")
        assert result.version == "4.22.2"
    ...
    def test_no_builds_raises(self, mock_explorer):
        ...
        with pytest.raises(ValueError, match="No released builds found"):
            find_released_source(explorer=mock_explorer, minor_version="v4.22")
    ...
    def test_candidate_only_raises(self, mock_explorer):
    ...
    def test_stable_preferred_over_candidate(self, mock_explorer):
        channels = [
            make_channel_info(channel="stable", released_to_prod=True, iib="iib:1"),
            make_channel_info(channel="candidate", released_to_prod=True, iib="iib:2"),
        ]
        build = make_released_build(csv_version="v4.22.2", version="v4.22.2.rhel9-31", channels=channels)
        mock_explorer.get_released_builds.return_value = [build]
        result = find_released_source(explorer=mock_explorer, minor_version="v4.22")
