# Release Checklist

Generated: 2026-07-08 06:08 UTC | Versions: 10

## 4.12 -> 4.12.25

**Target**: 4.12.25 (4.12.25-39) | channel: candidate | **in stage** | not released to prod

| Lane | Source | IIB | Channel | Post-Upgrade Suite |
|------|--------|-----|---------|-------------------|
| Z stream | 4.12.24 | iib:1149500 | stable | NONE |
| latest z | 4.12.0 | iib:399135 | stable | NONE |

## 4.14 -> 4.14.24

**Target**: 4.14.24 (4.14.24.rhel9-5) | channel: stable | **in stage** | not released to prod

| Lane | Source | IIB | Channel | Post-Upgrade Suite |
|------|--------|-----|---------|-------------------|
| Z stream | 4.14.23 | iib:1168279 | candidate | NONE |
| EUS | 4.12.24 | iib:1149500 | stable | UTS-Marker |
| latest z | 4.14.0 | iib:611376 | stable | NONE |

## 4.16 -> 4.16.40

**Target**: 4.16.40 (4.16.40.rhel9-5) | channel: stable | **in stage** | not released to prod

| Lane | Source | IIB | Channel | Post-Upgrade Suite |
|------|--------|-----|---------|-------------------|
| Z stream | 4.16.39 | iib:1167849 | candidate | NONE |
| EUS | 4.14.24 | iib:1173655 | candidate | UTS-Marker |
| latest z | 4.16.0 | iib:763432 | stable | NONE |

## 4.17 -> 4.17.53

**Target**: 4.17.53 (4.17.53.rhel9-11) | channel: stable | **in stage** | not released to prod

| Lane | Source | IIB | Channel | Post-Upgrade Suite |
|------|--------|-----|---------|-------------------|
| Y stream | 4.16.40 | iib:1172274 | candidate | UTS-Marker |
| Z stream | 4.17.52 | iib:1170160 | candidate | NONE |
| latest z | 4.17.0 | iib:840694 | stable | NONE |

## 4.18 -> 4.18.41

**Target**: 4.18.41 (4.18.41.rhel9-11) | channel: stable | **in stage** | not released to prod

| Lane | Source | IIB | Channel | Post-Upgrade Suite |
|------|--------|-----|---------|-------------------|
| Y stream | 4.17.53 | iib:1174057 | candidate | UTS-Marker |
| Z stream | 4.18.40 | iib:1165498 | candidate | NONE |
| EUS | 4.16.40 | iib:1172274 | candidate | NONE |
| latest z | 4.18.0 | iib:928381 | stable | NONE |

## 4.19 -> 4.19.28

**Target**: 4.19.28 (4.19.28.rhel9-14) | channel: stable | **in stage** | not released to prod

| Lane | Source | IIB | Channel | Post-Upgrade Suite |
|------|--------|-----|---------|-------------------|
| Y stream | 4.18.41 | iib:1174052 | candidate | UTS-Marker |
| Z stream | 4.19.27 | iib:1158970 | candidate | NONE |
| latest z | 4.19.0 | iib:991381 | stable | NONE |

## 4.20 -> 4.20.20

**Target**: 4.20.20 (4.20.20.rhel9-11) | channel: stable | **in stage** | not released to prod

| Lane | Source | IIB | Channel | Post-Upgrade Suite |
|------|--------|-----|---------|-------------------|
| Y stream | 4.19.28 | iib:1164900 | candidate | UTS-Marker |
| Z stream | 4.20.19 | iib:1165585 | candidate | NONE |
| EUS | 4.18.41 | iib:1174052 | candidate | NONE |
| latest z | 4.20.0 | iib:1063267 | stable | NONE |

## 4.21 -> 4.21.12

**Target**: 4.21.12 (4.21.12.rhel9-17) | channel: stable | **in stage** | not released to prod

| Lane | Source | IIB | Channel | Post-Upgrade Suite |
|------|--------|-----|---------|-------------------|
| Y stream | 4.20.20 | iib:1174090 | candidate | UTS-Marker |
| Z stream | 4.21.11 | iib:1164869 | candidate | NONE |
| latest z | 4.21.0 | iib:1104286 | stable | NONE |

## 4.22 -> 4.22.2

**Target**: 4.22.2 (4.22.2.rhel9-12) | channel: stable | **in stage** | not released to prod

| Lane | Source | IIB | Channel | Post-Upgrade Suite |
|------|--------|-----|---------|-------------------|
| Y stream | 4.21.12 | iib:1171682 | candidate | UTS-Marker |
| Z stream | 4.22.1 | iib:1164899 | candidate | NONE |
| EUS | 4.20.20 | iib:1174090 | candidate | NONE |
| latest z | 4.22.0 | iib:1159577 | stable | NONE |

## 4.23 -> 4.23.0

**Target**: 4.23.0 (4.23.0.rhel9-8) | channel: candidate | **in stage** | not released to prod

| Lane | Source | IIB | Channel | Post-Upgrade Suite |
|------|--------|-----|---------|-------------------|
| Y stream | 4.22.2 | iib:1170067 | candidate | UTS-FULL |
